...
export default function Show() {
    const { room, contract, auth } = usePage().props;
    const user = auth?.user;
    const isLandlord = user?.role === 'landlord';
    const userPerms = auth?.permissions || user?.permissions || [];
    const canEdit = isLandlord || userPerms.includes('contracts.edit');
    const canDelete = isLandlord || userPerms.includes('contracts.delete');
        ...
        <div className="min-h-screen bg-emerald-50/30 py-8 px-4 sm:px-6 lg:px-8 font-sans">
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex flex-wrap gap-3 justify-end border-t border-gray-200 pt-6">
                    {/* Gia hạn */}
                    {canEdit && contract.status === 'expired' && (
                        <button
            ...
            <ConfirmModal
                ...
                onConfirm={() => {
                    router.put(route('landlord.rooms.contracts.update', [room.id, contract.id]), { status: 'terminated' });
                }}
                title="Chấm dứt hợp đồng"
                message="Bạn có chắc chắn muốn chấm dứt hợp đồng này sớm không?"
            ...
            <ConfirmModal
                ...
                onConfirm={() => {
                    router.delete(route('landlord.rooms.contracts.destroy', [room.id, contract.id]));
                }}
                title="Xóa hợp đồng"
                message="Hành động này không thể hoàn tác. Bạn chắc chắn muốn xóa hợp đồng này?"
