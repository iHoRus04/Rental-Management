...
export default function RenterServices({ auth, renter, renterServices, allServices }) {
    ...
    const handleAddService = (e) => {
        e.preventDefault();
        post(route('landlord.renters.services.attach', renter.id), {
            onSuccess: () => {
                reset();
    ...
    const handleEditService = (e) => {
        e.preventDefault();
        put(route('landlord.renter-services.update', editingService.pivot.id), {
            onSuccess: () => {
    ...
    const handleDeleteService = (renterServiceId) => {
        if (confirm('Bạn có chắc chắn muốn gỡ dịch vụ này khỏi người thuê?')) {
            router.delete(route('landlord.renter-services.detach', renterServiceId));
        }
    };
    ...
    // Filter out services already attached to renter
    const availableServices = allServices.filter(
        service => !renterServices.some(rs => rs.id === service.id)
    );

    return (
        <AuthenticatedLayout user={auth.user}>
            <div className="p-6 md:p-10 max-w-[1400px] mx-auto font-sans">
                <Head title={`Dịch vụ - ${renter.name}`} />
                ...
                <div className="mb-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h1 className="text-3xl font-extrabold text-teal-900 tracking-tight">Dịch vụ - {renter.name}</h1>
                            <p className="text-emerald-600/80 font-medium text-sm mt-1">
                                Quản lý dịch vụ cho người thuê này
                            </p>
                        </div>
                ...
                {/* SERVICES LIST */}
                {renterServices.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {renterServices.map((service) => (
                            <div 
                                key={service.id} 
