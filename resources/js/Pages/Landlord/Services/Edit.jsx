...
export default function Edit({ auth, service }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('landlord.services.update', service.id));
    };

    return (
        <AuthenticatedLayout user={auth.user}>
            <div className="p-6 md:p-10 max-w-3xl mx-auto font-sans">
                <Head title={`Chỉnh sửa: ${service.name}`} />
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-extrabold text-teal-900 tracking-tight">Chỉnh sửa dịch vụ</h1>
                    <p className="text-emerald-600/80 font-medium text-sm mt-1">{service.name}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {processing ? 'Đang cập nhật...' : 'Cập nhật dịch vụ'}
                        </button>
                        <Link
                            href={route('landlord.services.index')}
